...
async def forward_card(
    ...
    try:
        ...
        # ✅ Old = FETCH_GROUP, Live = LIVE_GROUP
        if is_old:
            target = config.FETCH_GROUP
        else:
            if category == "live":
                target = config.LIVE_GROUP
            else:
                target = config.CHARGED_GROUP

        # ✅ Sirf target group me - Log channel NAHI
        await _safe_send(client, target, text)

        # ✅ Save to DB after forward
        db.save_card(
            card=card,
            source=source,
            chat_id=chat_id,
        ...
        try:
            db.save_skipped(
                card, source, chat_id, message_id, category
            )
